...
def lineCollides(p1, p2, obstacles=None):
    ...
    for circle in obstacles:
        ...
        if manhattan_dist(cen,(h,k)) < 1.1*rad:
    ...
    while True:
        if currentState == 'init':
            ...
            if count < NUMNODES:
                ...
                if(manhattan_dist(parentNode.point, newnode) <= GOAL_RADIUS):
                    foundNext = False
                    continue
                for p in nodes:
                    if manhattan_dist(p.point,newnode) + p.cost <= manhattan_dist(parentNode.point,newnode) + parentNode.cost:
                            if lineCollides(newnode,p.point) == False and pointCollides(newnode) == False:
                                parentNode = p
				
                newNode = Node(newnode, parentNode,parentNode.cost+manhattan_dist(newnode,parentNode.point))
                ...
                if point_circle_collision(newnode, goalPoint.point, GOAL_RADIUS) or lineCollides(newnode, parentNode.point, (goalPoint.point, GOAL_RADIUS)):
                    ...
                    goalNode.cost = parentNode.cost + manhattan_dist(goalNode.point, parentNode.point)
                    print(goalNode.cost)
                    ...
                    goalNode.cost = newNode.cost + manhattan_dist(goalNode.point, newNode.point)
